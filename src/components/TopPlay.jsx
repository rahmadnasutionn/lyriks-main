...
const TopPlay = () => {
  ...
  return (
    <div ref={divRef} className="xl:ml-6 ml-0 xl:mb-0 mb-6 flex-1 xl:max-w-[500px] flex flex-col max-w-full">
      <div className="w-full flex flex-col">
        <div className="flex flex-row justify-between items-center">
          <h2 className="text-white font-bold text-2xl">Top Charts</h2>
          <Link to="/top-charts">
            <p className="cursor-pointer text-gray-300 hover:text-gray-600 hover:underline text-base">See more</p>
          </Link>
        </div>
      ...
      <div className="w-full flex flex-col my-8">
        ...
        <Swiper slidesPerView="auto" spaceBetween={15} freeMode centeredSlides centeredSlidesBounds modules={[FreeMode]} className="mt-4">
          {topPlays?.map((song, i) => (
            <SwiperSlide key={song?.key} i={i} style={{ width: '25%', height: 'auto' }} className="shadow-lg rounded-full animate-slideright">
              <Link to={`/artists/${song?.artists[0].adamid}`}>
